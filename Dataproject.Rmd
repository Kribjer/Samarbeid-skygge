...
#### Renewable energy useage Per Capita  
  ...
  When we look at which countries has the biggest consumption per capita the list changes, only Canada is mention on both rankings but only on second place. Norway tops the ranking, but this is not strange at all considering that the county get 99% of its energy from renewable energy (mainly hydroelectric).  
...
### Average growth-rate in the usage of renewable energy from 65' to 16'  
  
  Now we want to look at which countries has the highest growth rate in consumption of renewable energy. The reason we wanted to look at this is because we were wondering if its only “well developed” countries that pushes the technology forward to increase the amount of produced and used renewable energy. We made two datasets, just like the total consumption in the world. Top 5 in the world total, and Top 5 per capita.  
    To find which countries was in the top 5 in each category, we had to use the average growth rate. We did this to eliminate the problem where a county could have had an insane growth rate in 1970 and close to zero for all the other years. By doing it this way we think the data is more accurate in the representation on which country has the highest growth rate. 
