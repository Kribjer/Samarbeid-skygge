...
Headprcap <- totalprcap[order(-totalprcap$Prcap), ]
head(unique(Headprcap$Country, 5))

Headtotal <- energyview[order(-energyview$TotalenergyUse), ] 
Headtotal
...
avgGrowth <- rateprcap %>%
  group_by(Country) %>%
  summarise(AvgGrowth = sum(Prcap/51))
...
graphing <- function(x){ggplot(x, aes(x = Year, y = x[ ,3])) +
  geom_line()}
...
filtcountry <- function(x){x%>%filter(Country == unique(x[ ,1]))}
...
test5 <- energyview %>%
  group_by(Country) %>%
  order(-energyview$TotalenergyUse)
```
