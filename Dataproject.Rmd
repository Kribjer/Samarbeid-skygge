...
#Trying to create a function that graphs automatically.
...
test5 <- energyview[order(-energyview$TotalenergyUse), ]
...
test6 <- energyview %>%
  plot_ly(type = "scatter", x = ~Year, y = ~TotalenergyUse, text = ~Country, hoverinfo = "text", mode = "markers", transforms = list(
    list(
      type = "filter",
      target = ~Country,
      operation = "=", 
      value = unique(energyview$Country)[1]
    )
  )) %>%
  layout(
    updatemenus = list(
      list(
        ...
        buttons = list(
          list(method = "restyle",
            args = list("transforms[0].value", unique(energyview$Country)[1],
             label = unique(energyview$Country)[1]),
            list(method = "restyle",
            args = list("transforms[0].value", unique(energyview$Country)[2],
             label = unique(energyview$Country)[2]), 
            list(method = "restyle",
            args = list("transforms[0].value", unique(energyview$Country)[3],
             label = unique(energyview$Country)[3]))))))))
